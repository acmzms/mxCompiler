...
public class IRBuilder implements IAstVisitor {
    ...
    private void buildInitFunction(CompilationUnit node) {
    ...
    @Override public void visit(CompilationUnit node) {
        for (VariableDeclaration variableDeclaration : node.globalVariables) {
            StaticData data = new StaticData(variableDeclaration.name, REGISTER_WIDTH);
    ...
    /*private boolean deserveBackOptimization(FuncDeclaration funcDeclaration) {
        ...
        if (funcDeclaration.name.equals("func")) {
            return false;
        }
        return  useBackupOptimization
    ...
    @Override public void visit(ArrayExpression node) {
        ...
        if (index instanceof Immediate) {
            memory = new Memory(base, new Immediate(((Immediate) index).value * REGISTER_WIDTH + REGISTER_WIDTH));
        } else if(index instanceof Register) {
            memory = new Memory(base, (Register) index, REGISTER_WIDTH, new Immediate(REGISTER_WIDTH));
        } else if(index instanceof Memory){
            VirtualRegister vr = new VirtualRegister("");
            curBB.append(new Move(curBB, vr, index));
            memory = new Memory(base, vr, REGISTER_WIDTH, new Immediate(REGISTER_WIDTH));
        } else {
            assert false;
            memory = null;
        }
        ...
        if (dims.size() == 0) {
            if (baseBytes == 0) {
                ...
                if (constructor != null) {
                    ...
                    if (baseBytes == REGISTER_WIDTH) {
                        curBB.append(new Move(curBB, new Memory(retAddr), new Immediate(0)));
                    } else if (baseBytes == REGISTER_WIDTH * 2) {  //  maybe string
                        curBB.append(new BinaryInst(curBB, BinaryInst.BinaryOp.ADD, retAddr, new Immediate(REGISTER_WIDTH)));
                        curBB.append(new Move(curBB, new Memory(retAddr), new Immediate(0)));
                        curBB.append(new BinaryInst(curBB, BinaryInst.BinaryOp.SUB, retAddr, new Immediate(REGISTER_WIDTH)));
                    }
                }
                return retAddr;
            }
            ...
            curBB.append(new Lea(curBB, bytes, new Memory(size, REGISTER_WIDTH, new Immediate(REGISTER_WIDTH))));
            ...
            if (dims.size() == 1) {
                ...
                curBB.append(new Move(curBB, new Memory(addr, size, REGISTER_WIDTH), pointer));
            } else {
    ...
    private int countOperations(Statement statement) {
        ...
        if (statement instanceof IfStatement) {
            ...
            count += countOperations(((ExpressionStatement) statement).expression);
        } else if (statement instanceof VariableDeclarationStatement) {
            count += countOperations(((VariableDeclarationStatement) statement).declaration.init);
        } else {
            count += 1;
        }
        return count;
